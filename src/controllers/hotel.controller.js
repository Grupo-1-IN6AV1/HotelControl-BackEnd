...
exports.saveHotel  = async (req, res)=>{
    try{
        ...
        const existHotel = await Hotel.findOne({name: data.name});
        if(!existHotel){
            const hotel= new Hotel(data);
            await hotel.save();
            return res.send({message: 'Hotel saved', hotel});
...
exports.updateHotel= async (req, res)=>{
    try{
        ...
        if(!msg)
        {
            ...
            return res.send({message: 'Update Hotel', updatedHotel});
...
exports.deleteHotel= async (req, res)=>{
    try{
        const hotelId = req.params.id;
        const hotelExist = await Service.findOne({_id: hotelId});
        ...
        const hotelDeleted = await Service.findOneAndDelete({_id: hotelId});
        return res.send({message: 'Delete Hotel.', hotelDeleted });
